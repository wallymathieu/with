...
namespace Tests.Ranges
{
	public class RangeTests
	{
		...
		public void Decimal_range_has()
		{
			...
			Assert.Equal(range.Contains(2.1m), true);
			Assert.Equal(range.Contains(0.1m), true);
			Assert.Equal(range.Contains(4.1m), true);

			Assert.Equal(range.Contains(-1.1m), false);
			Assert.Equal(range.Contains(1.1m), false);
			Assert.Equal(range.Contains(5.1m), false);

			Assert.Equal(range.Contains(2m), false);
			Assert.Equal(range.Contains(0m), false);
			Assert.Equal(range.Contains(4m), false);
		}
	}
}
